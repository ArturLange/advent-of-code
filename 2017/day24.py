...
def get_bridge_strength_with_sames(bridge: Bridge) -> int:
    all_values = set()
    for comp in bridge:
        all_values = all_values.union({comp.start, comp.end})
    return get_bridge_strength(bridge) + sum((x*2 for x in all_values if x in sames))
...
class BridgeTree:
    __slots__ = ['max_value', 'bridge', 'value']

    def __init__(self, bridge: Bridge, max_value: Dict[str, int]):
        self.max_value = max_value
        self.bridge: Bridge = bridge
        self.value: int = get_bridge_strength_with_sames(self.bridge)
        self.max_value['max'] = max((self.value, self.max_value['max']))
